...
object OMSRawToProcessed extends OMSRunner with OMSInitializer {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val lastProcessedRawActionsVersion = Try {
      spark.read.format("delta")
           ...
           .select("lastVersion").as[Long].head() }.getOrElse(0L)
    ...
    if(!commitSnapshotExists){
      ...
      commitInfoSnapshotTable match {
        ...
        case Failure(ex) => throw new RuntimeException(s"Unable to update the Commit Info Snapshot table. $ex")
      }
    }
    ...
    if(!actionSnapshotExists) {
      ...
      actionSnapshotTable match {
        ...
        case Failure(ex) => throw new RuntimeException(s"Unable to update the Action Snapshot table. $ex")
      }
    }


    val updatedRawActionsLastProcessedVersion = Seq((s"${omsConfig.rawActionTable}",currentRawActionsVersion, Instant.now()))
    ...
    processedHistoryTable match {
      ...
      case Failure(ex) => throw new RuntimeException(s"Unable to update the Processed History table. $ex")
    }
  }
  ...
  def prepareAddRemoveActionsFromRawActions(rawActions: org.apache.spark.sql.DataFrame) = {
    val addFileActions = rawActions
      ...
      .selectExpr("add","remove",PUID,s"$PATH as data_path",COMMIT_VERSION,COMMIT_TS,COMMIT_DATE)

    val duplicateAddWindow = Window.partitionBy(col(PUID),col("add.path")).orderBy(col(COMMIT_VERSION).desc_nulls_last)
    //Duplicate AddFile actions could be present under rare circumstances
    val rankedAddFileActions = addFileActions.withColumn("rank", rank().over(duplicateAddWindow))
    val dedupedAddFileActions = rankedAddFileActions.where("rank = 1").drop("rank")
    ...
    val addRemoveFileActions = dedupedAddFileActions.unionByName(removeFileActions)
      .select(col(PUID), col("data_path"),col(COMMIT_VERSION), col(COMMIT_TS),col(COMMIT_DATE),
  ...
  def computeCumulativeFilesFromAddRemoveActions(addRemoveActions: org.apache.spark.sql.DataFrame) = {
    val commitVersions = addRemoveActions.select(PUID,COMMIT_VERSION,COMMIT_TS,COMMIT_DATE).distinct()
    val cumulativeAddRemoveFiles = addRemoveActions.as("arf").join(commitVersions.as("cv"),col("arf.puid") === col("cv.puid")
      && col(s"arf.$COMMIT_VERSION") <= col(s"cv.$COMMIT_VERSION"))
      .select(col(s"cv.$COMMIT_VERSION"),col(s"cv.$COMMIT_TS"),col(s"cv.$COMMIT_DATE"),
        col(s"arf.$PUID"),col("arf.data_path"),
        col("arf.add_file"),col("arf.remove_file"))
    cumulativeAddRemoveFiles
  }

  def deriveActionSnapshotFromCumulativeActions(cumulativeAddRemoveFiles: org.apache.spark.sql.DataFrame) = {
    val cumulativeAddFiles = cumulativeAddRemoveFiles.where(col("add_file.path").isNotNull).drop("remove_file")
    val cumulativeRemoveFiles = cumulativeAddRemoveFiles.where(col("remove_file.path").isNotNull).drop("add_file")
    val snapshotInputFiles = cumulativeAddFiles.as("ca").join(cumulativeRemoveFiles.as("cr"),
      col(s"ca.$PUID") === col(s"cr.$PUID")
        && col(s"ca.$COMMIT_VERSION") === col(s"cr.$COMMIT_VERSION")
        && col("ca.add_file.path") === col("cr.remove_file.path"),"leftanti").selectExpr("ca.*")
    snapshotInputFiles
  }

  def computeActionSnapshotFromRawActions(rawActions: org.apache.spark.sql.DataFrame, snapshotExists: Boolean) = {
    ...
    val cumulativeAddRemoveFiles = if(snapshotExists){
      ...
      val previousSnapshotMaxCommitVersion = previousSnapshot.groupBy(PUID).agg(max(COMMIT_VERSION).as(COMMIT_VERSION))
      ...
      val cumulativeAddRemoveFileActions = computeCumulativeFilesFromAddRemoveActions(addRemoveFileActions
          .unionByName(previousSnapshotMaxAddRemoveFileActions))
